#!/usr/bin/env python3
"""Extrae fichas medicas PDF, valida, ejecuta ETL y entrena/evalua modelos."""
...
from scripts import _03_train_model_loader  # type: ignore
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Extrae fichas medicas PDF, valida, ejecuta ETL y genera metricas del modelo"
    )
    parser.add_argument(
        "--input-dir",
        type=Path,
    ...
    logger.info("[1/4] Extrayendo fichas medicas desde %s", args.input_dir)
    ...
    if not args.skip_validation:
        logger.info("[2/4] Validando CSV contra %s", args.validate_against)
        validate_employees_csv_match(args.validate_against, args.output_csv)
        logger.info("Validacion exacta OK")
    else:
        logger.info("[2/4] Validacion omitida")

    logger.info("[3/4] Ejecutando ETL con %s", args.output_csv)
    run_pipeline(employees_path=args.output_csv)

    logger.info("[4/4] Entrenando modelos y generando metricas")
